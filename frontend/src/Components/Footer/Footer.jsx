        ...
        <footer className="bg-black text-white pt-16 pb-8 border-t border-purple-900/35 z-10 relative">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-16">
                    ...
                    {/* Quick Links */}
                    <div className="flex flex-col items-center md:items-start">
                        <h3 className="text-xl font-bold mb-6 text-white text-center md:text-left">Quick Links</h3>
                        <ul className="space-y-3 w-full flex flex-col items-center md:items-start">
                            {['Home', 'Sponsors', 'Events', 'Timeline', 'Our Team', 'Gallery', 'Contact'].map((item) => (
                                <li key={item} className="flex items-center group cursor-pointer hover:translate-x-2 transition-transform duration-300">
                                    <span className="w-1.5 h-1.5 rounded-full bg-purple-500 mr-2 group-hover:scale-150 transition-transform"></span>
                                    <a href={item === 'Home' ? '/' : `/${item.toLowerCase().replace(' ', '')}`} className="text-gray-400 group-hover:text-purple-400 transition-colors">
                                        {item}
                                    </a>
                                </li>
                            ))}
                    ...
                    <div className="flex flex-col items-center md:items-start">
                        ...
                        <div className="flex flex-col gap-6 items-center md:items-start text-center md:text-left">
                            ...
                            <div className="flex items-center gap-4 group">
                                ...
                                <a href="mailto:ingenium@ahduni.edu.in" className="text-gray-400 hover:text-purple-400 transition-colors text-sm">
                                    spu.ac.in
                                </a>
                            </div>

                            <div className="flex gap-4 mt-2">
                                <a href="#" className="p-2 rounded-full bg-purple-900/20 hover:bg-purple-500 hover:text-white text-purple-400 transition-all duration-300 transform hover:scale-110">
                ...
                <div className="text-center text-xs text-gray-500 uppercase tracking-widest flex flex-col md:flex-row justify-center items-center gap-2">
                    ...
                    <span className="text-purple-500 font-semibold mx-1">Jaypal Singh</span>
                    <span className="hidden md:inline text-purple-900">|</span>
                    <span className="text-purple-500 font-semibold mx-1">Patel Jainish</span>
                </div>
            </div>
        </footer>
    );
